...
export class GroupMemberService {
    ...
    static async getGroupContacts(
        ...
        const nextCursor = memberships.length === limit ? memberships[memberships.length - 1].contactId : null;
    ...
    static async addGroupMembers(
        ...
        return await prisma.$transaction(async (tx) => {
            ...
            if (validContactsCount !== uniqueContactIds.length) {
                throw new Error("One or more contacts are invalid or do not belong to this workspace.");
            }
