...
import { validateCustomFields, MAX_CUSTOM_FIELDS_PER_ORG } from "../utils/contact-fields";
...
export class ContactService {
    ...
    static async createContact(
        ...
        if ((!data.phoneE164 || data.phoneE164.length === 0) && (!data.email || data.email.length === 0)) {
            throw new Error("A contact must have at least an email or a phone number.");
        }

        if (data.customFields && Object.keys(data.customFields).length > 0) {
            if (Object.keys(data.customFields).length > MAX_CUSTOM_FIELDS_PER_ORG) {
                throw new Error(`Maximum ${MAX_CUSTOM_FIELDS_PER_ORG} custom fields allowed per contact`);
            }
    ...
    static async updateContact(
        ...
        const resultingPhone = data.phoneE164 !== undefined ? data.phoneE164 : existingContact.phoneE164;
        const resultingEmail = data.email !== undefined ? data.email : existingContact.email;
        ...
        if (!hasResultingPhone && !hasResultingEmail) {
            throw new Error("A contact must have at least an email or a phone number. You cannot remove both.");
        }
        ...
        return prisma.contact.update({
            ...
            data: {
                ...data,
                customFields: data.customFields === undefined ? undefined : (data.customFields ? (data.customFields as Prisma.InputJsonValue) : Prisma.DbNull),
            },
        });
    }
    ...
    static async getContactsBySegment(
        ...
        const segment = await SegmentService.getSegmentById(organizationId, segmentId);

        // Compile the dynamic AST JSON into a Prisma Where clause
        const baseWhere = compileSegmentToPrismaWhere(organizationId, segment.definition as any);
        ...
        const [contacts, total] = await Promise.all([
            prisma.contact.findMany({
                ...
                orderBy: { createdAt: "desc" },
                select: { id: true, name: true, phoneE164: true, email: true, gender: true } // Safe basic DTO
            }),
