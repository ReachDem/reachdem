...
describe("Groups API (Core Services)", () => {
    ...
    describe("Group Creation", () => {
        it("should return a rejection if group name already exists", async () => {
            ...
            await expect(GroupService.createGroup(MOCK_ORG_ID, { name: "VIPs" }))
                .rejects.toThrow('A group with the name "VIPs" already exists in this workspace.');
        });
        ...
        it("should successfully create a new group if valid", async () => {
            ...
            const result = await GroupService.createGroup(MOCK_ORG_ID, { name: "New Group" });
