...
export const DataTableColumnHeader = <TData, TValue>({
  ...
  if (!canSort) {
    return (
      <span className="text-foreground flex h-8 items-center text-xs font-medium sm:text-sm">
        {title}
      </span>
    );
  }
  ...
  return (
    <Button
      variant="ghost"
      size="sm"
      className="text-foreground flex h-8 items-center gap-2 px-0 text-xs font-medium sm:text-sm"
...
export const columns: ColumnDef<z.infer<typeof schema>>[] = [
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <span className="text-muted-foreground font-mono text-xs tracking-wide whitespace-nowrap uppercase">
          {sku}
        </span>
      );
    },
  ...
  return (
    <section className={cn("py-24", className)}>
      <div className="container px-4 sm:px-6 lg:px-8">
                      ...
                      {headerGroup.headers.map((header) => {
                        return (
                          <TableHead
                            ...
                            className="border-border bg-muted/50 relative h-12 border-y px-3 text-left text-xs font-medium select-none first:rounded-l-lg first:border-l first:pl-3 last:rounded-r-lg last:border-r last:pr-3 sm:px-4 sm:text-sm sm:first:pl-5 sm:last:pr-5"
                          >
                  ...
                  {table.getRowModel().rows?.length ? (
                    table.getRowModel().rows.map((row) => (
                      <TableRow
                        ...
                        className="hover:bg-muted/50 border-0 [&:first-child>td:first-child]:rounded-tl-lg [&:first-child>td:last-child]:rounded-tr-lg [&:last-child>td:first-child]:rounded-bl-lg [&:last-child>td:last-child]:rounded-br-lg"
                      >
        ...
        <p className="text-muted-foreground mt-2 text-center text-xs sm:hidden">
          ← Swipe to see more →
        </p>
      </div>
    </section>
