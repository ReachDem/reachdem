...
type ArpuDay = { day: string; arpu: number };
...
export const getArpu = unstable_cache(
  async () => {
    ...
    const data: ArpuDay[] = rows.map((r: ArpuDayRow) => {
...
type DailyActivePoint = { day: string; value: number };
...
export const getDailyActiveOrgs = unstable_cache(
  async () => {
    ...
    const curData: DailyActivePoint[] = current.map((r: DailyActiveRow) => ({
      day: r.day,
      value: Number(r.orgs),
    }));
    const prevData: DailyActivePoint[] = previous.map((r: DailyActiveRow) => ({
      day: r.day,
      value: Number(r.orgs),
    }));
...
export const getClientsByPlan = unstable_cache(
  async () => {
    ...
    const items: PlanDistItem[] = rows.map((r: PlanDistRow) => ({
