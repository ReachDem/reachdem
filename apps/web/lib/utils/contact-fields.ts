import { ContactFieldDefinition } from "@prisma/client";
...
export function validateCustomFields(
    ...
    for (const [key, value] of Object.entries(customFields)) {
        ...
        if (!def) {
            return { isValid: false, error: `Custom field key '${key}' is not defined for this workspace` };
        }
        ...
        if (!isValid) {
            return { isValid: false, error: `Invalid value for custom field '${key}'. Expected type: ${def.type}` };
        }
    }
