...
describe("Segments API - REAL DATABASE INTEGRATION", () => {
  ...
  it("should physically create a segment with a valid JSON AST", async () => {
    ...
    const response = await createSegment(req as any, {
  ...
  it("should reliably evaluate contacts that match the dynamic segment criteria", async () => {
    ...
    const req = new NextRequest(
      `http://localhost:3000/api/v1/segments/${primarySegmentId}/contacts`
    );
    const response = await listSegmentContacts(req as any, {
      params: Promise.resolve({ id: primarySegmentId }),
    });
  ...
  it("should actively evaluate Contacts via Postgres JSONB custom fields", async () => {
    ...
    const respCreate = await createSegment(reqCreate as any, {
      params: Promise.resolve({} as Record<string, string>),
    });
    ...
    // Fetch Evaluation
    const reqEval = new NextRequest(
      `http://localhost:3000/api/v1/segments/${customSegmentId}/contacts`
    );
    const respEval = await listSegmentContacts(reqEval as any, {
      params: Promise.resolve({ id: customSegmentId }),
    });
  ...
  it("should delete the segment successfully", async () => {
    const req = new NextRequest(
      `http://localhost:3000/api/v1/segments/${primarySegmentId}`,
      { method: "DELETE" }
    );
    const response = await deleteSegment(req as any, {
      params: Promise.resolve({ id: primarySegmentId }),
    });
