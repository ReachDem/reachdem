...
describe('Groups API - REAL DATABASE INTEGRATION', () => {
    ...
    describe('Integration: Group CRUD', () => {
        ...
        it('should physically update the group description', async () => {
            ...
            const req = new NextRequest(`http://localhost:3000/api/v1/groups/${groupId}`, {
                method: 'PATCH',
                body: JSON.stringify({
            ...
            const { data } = await response.json();
            expect(data.description).toBe('Updated description for integration tests');
        });
        ...
        it('should physically delete the group', async () => {
            ...
            const req = new NextRequest(`http://localhost:3000/api/v1/groups/${tempId}`, {
                method: 'DELETE',
            });
    ...
    describe('Integration: Bulk Memberships', () => {
        ...
        beforeEach(async () => {
            ...
            if (createdGroupIds.length === 0) {
                const newGrp = await prisma.group.create({ data: { name: "Bulk Test Base", organizationId: REAL_ORG_ID } });
        ...
        it('should successfully bulk insert contacts into a group', async () => {
            ...
            const req = new NextRequest(`http://localhost:3000/api/v1/groups/${targetGroupId}/contacts`, {
                method: 'POST',
        ...
        it('should reject bulk insertions that attempt to span workspaces', async () => {
            ...
            await prisma.organization.delete({ where: { id: "evil-org-2" } }).catch(() => { });
        });

        it('should fail elegantly if bulk payload exceeds 10000 contacts', async () => {
            const massiveArray = Array.from({ length: 10001 }, (_, i) => `fake_id_${i}`);
