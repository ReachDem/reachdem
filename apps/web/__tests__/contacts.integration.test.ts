...
import { POST as createContactHandler, GET as getContactsHandler } from "../app/api/v1/contacts/route";
import { POST as createContactFieldHandler, GET as getContactFieldsHandler } from "../app/api/v1/contact-fields/route";
import { PATCH as updateContactHandler, DELETE as deleteContactHandler } from "../app/api/v1/contacts/[id]/route";
import { PATCH as updateContactFieldHandler, DELETE as deleteContactFieldHandler } from "../app/api/v1/contact-fields/[id]/route";
...
describe("Contacts API - REAL DATABASE INTEGRATION", () => {
    ...
    if (!REAL_ORG_ID || !TEST_USER_ID || !TEST_USER_EMAIL) {
        throw new Error("Missing required test environment variables: TEST_ORG_ID, TEST_USER_ID, or TEST_USER_EMAIL");
    }
    ...
    describe("Integration: Contact Creation", () => {
        ...
        it("should physically retrieve the contacts from the real database", async () => {
            const req = new NextRequest("http://localhost:3000/api/v1/contacts?limit=10", {
                method: "GET"
            });
        ...
        it("should reject contact creation if BOTH phone and email are missing", async () => {
            ...
            expect(JSON.stringify(json.error)).toContain("Either phone number or email");
        });
        ...
        it("should physically update the contact in the database", async () => {
            ...
            const createReq = new NextRequest("http://localhost:3000/api/v1/contacts", {
                method: "POST",
                body: JSON.stringify(payload)
            });
            ...
            const updateReq = new NextRequest(`http://localhost:3000/api/v1/contacts/${contactId}`, {
                method: "PATCH",
        ...
        it("should reject a contact update if it removes both phone and email", async () => {
            ...
            expect(JSON.stringify(json.error)).toContain("A contact must have at least an email or a phone number");
        });
        ...
        it("should physically soft-delete the contact in the database", async () => {
            ...
            const delReq = new NextRequest(`http://localhost:3000/api/v1/contacts/${contactId}`, {
                method: "DELETE"
            });
    ...
    describe("Integration: Contact Fields CRUD", () => {
        ...
        it("should update a contact field definition", async () => {
            ...
            const req = new NextRequest(`http://localhost:3000/api/v1/contact-fields/${testFieldId}`, {
                method: "PATCH",
                body: JSON.stringify(payload)
            });
        ...
        it("should fail to create a contact field when the organization limit (5) is reached", async () => {
            ...
            // Create them all
            const createPromises = dummyFields.map(f => prisma.contactFieldDefinition.create({ data: f as any }));
            ...
            const json = await res.json();
            expect(json.error).toContain("Maximum number of custom fields (5) reached");
        });
    });
