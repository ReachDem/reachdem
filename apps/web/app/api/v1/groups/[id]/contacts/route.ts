...
export const GET = withWorkspace<{ id: string }>(
  async ({ req, params, organizationId }) => {
    try {
      ...
      const result = await GroupMemberService.getGroupContacts(
        groupId,
        organizationId,
        { limit, cursor }
      );
...
export const POST = withWorkspace<{ id: string }>(
  async ({ req, params, organizationId }) => {
    try {
      ...
      try {
        const totalInserted = await GroupMemberService.addGroupMembers(
          groupId,
          organizationId,
          requestedContactIds
        );
        ...
        if (error instanceof Error) {
          if (
            ...
            error.message.includes("No Group found")
          ) {
            return NextResponse.json(
...
export const DELETE = withWorkspace<{ id: string }>(
  async ({ req, params, organizationId }) => {
    try {
      ...
      try {
        const totalDeleted = await GroupMemberService.removeGroupMembers(
          groupId,
          organizationId,
          requestedContactIds
        );
