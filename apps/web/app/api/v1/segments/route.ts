...
export const GET = withWorkspace(async ({ req, organizationId }) => {
    try {
        ...
        const result = await SegmentService.getSegments(organizationId, limit, cursor || undefined);
...
export const POST = withWorkspace(async ({ req, organizationId }) => {
    try {
        ...
        if (!result.success) {
            return NextResponse.json({ error: "Bad Request", details: result.error.format() }, { status: 400 });
        }
