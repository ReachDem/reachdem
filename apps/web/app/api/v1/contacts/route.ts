...
export const GET = withWorkspace(async ({ req, organizationId }) => {
    try {
        ...
        const result = await ContactService.getContacts(organizationId, { q, page, limit });
...
export const POST = withWorkspace(async ({ req, organizationId }) => {
    try {
        ...
        const contact = await ContactService.createContact(organizationId, validatedData);
        ...
        console.error("[Contacts_POST_ERROR_DEBUG]", error instanceof Error ? error.message : error);
        return NextResponse.json({ error: "Internal Server Error" }, { status: 500 });
    }
});
